...
export function extractText(html){const $=load(html);$('script,style,nav,header,footer,noscript,svg,form').remove();const scope=$('main').length?$('main').first():$('body');scope.find('p,h1,h2,h3,h4,li,br,div,section').append('\n');return scope.text().split('\n').map(s=>s.replace(/\s+/g,' ').trim()).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join('\n');}
